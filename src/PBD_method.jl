...
function PBD(a, ω, b; min_P_submod=0, tvd_min=0, P1_auto=true, P1_weight_min=1, heuristic=true, display=0, limit_time=3600, stats_PBD=[], return_stats=true)  
    ...
    if(P2 > 0)
        aω2 = a2.*ω2
        margin_loss_all_open = sum(max.(0, 2*aω2 .- sum(aω2, dims=2)), dims=1)[1,:] #margin_loss_all_open[d] = ρ_d(D\{d})
        margin_gain_all_closed = sum(aω2,dims=1)[1,:] #margin_loss_all_closed[d] = ρ_d(∅) #only for SC2
        function callback_submodular_cuts(cb_data)
            ...
            cover_p2 = a2*x_val #cover_p[p] = # of active locations covering profile p
            is_covered_p2 = min.(1, cover_p2) #is_covered_p[p] = 1 if profile p is covered by at least one active location
            ω2_covered = ω2.*is_covered_p2
            sum_ω2_covered = sum(ω2_covered)
            if(Θ_val > sum_ω2_covered)
                stats_PBD.n_submodular_cuts = stats_PBD.n_submodular_cuts + 2
                margin_gain_inactive = sum( (a2[:,inactive] .> is_covered_p2).*ω2, dims=1)[1,:] #margin_gain_inactive[i] = ρ_d(Dx), where d is the i-th inactive location and x is the current solution
                margin_loss_active = sum(min.(a2[:,active], a2[:,active].==cover_p2).*ω2, dims=1)[1,:] #margin_loss_active[i] = ρ_d(Dx\{d}), where d is the i-th active location and x is the current solution

                # submodularity cut (29)
                cut1 = @build_constraint(Θ <= sum_ω2_covered + sum(margin_gain_inactive[i]*x[d] for (i,d) in enumerate(inactive)) - sum(margin_loss_all_open[d]*(1-x[d]) for d in active))
                MOI.submit(m, MOI.LazyConstraint(cb_data), cut1)

                # submodularity cut (30)
                cut2 = @build_constraint(Θ <= sum_ω2_covered + sum(margin_gain_all_closed[d]*x[d] for d in inactive) - sum(margin_loss_active[i]*(1-x[d]) for (i,d) in enumerate(active)))
                MOI.submit(m, MOI.LazyConstraint(cb_data), cut2)

                #### submodularity cut (31), strengtened version of (30), not used in the experiments ###
                #gain_inactive_covered_at_most_1 = sum((a2[:,inactive].*(cover_p2.==is_covered_p2)).*ω2, dims=1)[1,:] #gain_inactive_covered_at_most_1[i] = weight of profiles covered by d and at most one active location (d is the i-th inactive location)
                #cut2 = @build_constraint(Θ <= sum_ω2_covered + sum(gain_inactive_covered_at_most_1[i]*x[d] for (i,d) in enumerate(inactive)) - sum(margin_loss_active[i]*(1-x[d]) for (i,d) in enumerate(active)))
                #MOI.submit(m, MOI.LazyConstraint(cb_data), cut2)
            end
        end
